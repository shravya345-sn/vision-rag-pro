...
from rag.retriever import process_documents, search_context
from rag.llm import get_answer
from rag.vision import get_image_description
import os

# 1. PAGE CONFIG & MODERN UI (Fixes layout and visibility)
st.set_page_config(page_title="VisionRAG Pro", layout="wide", page_icon="🧠")

st.markdown("""
    <style>
    /* 1. Main Background & Text Visibility Fix */
    .stApp { background: linear-gradient(135deg, #0f0c29 0%, #302b63 100%); color: #ffffff !important; }
    
    /* 2. CHAT BUBBLE FIX: High contrast text and glassmorphism */
...
# 3. SIDEBAR: MULTI-FILE SOURCE TRACKING (Order & Identity Fix)
with st.sidebar:
    ...
    if st.button("🚀 Global Sync"):
        # We store files in a list to preserve order and metadata
        pdf_pairs = []
        img_pairs = []
        
        # Collect PDF name and content independently
        for pdf in uploaded_pdfs:
            reader = PdfReader(pdf)
            text = "\n".join([page.extract_text() for page in reader.pages if page.extract_text()])
            pdf_pairs.append((pdf.name, text))
        
        # Collect Image name and AI description independently
        ...
        if pdf_pairs or img_pairs:
            with st.spinner("Building Intelligent Brain..."):
                # Sending pairs to retriever preserves document identity
...
# Display historical messages (Forced visibility)
    ...
    if st.session_state.vector_db:
        with st.chat_message("assistant"):
            with st.spinner("Analyzing cross-document context..."):
                # context now identifies each file by name
